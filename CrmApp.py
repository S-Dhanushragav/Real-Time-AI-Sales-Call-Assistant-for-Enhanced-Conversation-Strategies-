...
def fetch_latest_history_from_sheet(_sheet):
    ...
    try:
        ...
        header = ["Timestamp", "Client Email", "Transcript", "Sentiment", "Polarity", "LLM Summary"]
        ...
        for row in reversed(history_rows):
            history_list.append({
                ...
                "transcript": row[2] if len(row) > 2 else "N/A",
                "sentiment": row[3] if len(row) > 3 else "N/A",
                "summary": row[5] if len(row) > 5 else "N/A",
            })
        return history_list
    except Exception as e:
...
# --- NEW: FETCH CLIENT DATA BY EMAIL ---
def get_client_data_from_excel(email, file_path="Expanded_CRM_Data.xlsx"):
    try:
        df = pd.read_excel(file_path)
        df.columns = df.columns.str.strip()

        # Auto-detect email column (e.g., "Email", "Customer Email", "Email ID")
        email_col = None
        ...
        if not email_col:
            st.error("No email column found in Excel.")
            return None

        client_row = df[df[email_col].str.strip().str.lower() == email.strip().lower()]
...
# --- NEW: SUMMARIZE CLIENT DATA ---
def summarize_client_data(client_data):
    if not client_data:
        return "No matching client found for this email."
    prompt = f"""
    You are an AI sales assistant. Below is a client's CRM data:

    {client_data}

    Summarize this client profile in a structured, professional way.
    Format your response with bullet points:
    - Key Details (name, company, location if available)
    - Interests / Product of Interest
    - Needs / Pain Points
    - Deal Stage
...
def log_to_google_sheet(full_transcript, final_sentiment, final_polarity, final_summary):
    ...
    try:
        ...
        row_data = [
            ...
            st.session_state.get("client_email", "N/A"),
            full_transcript,
            final_sentiment,
            final_polarity,
            final_summary
...
with st.sidebar:
    ...
    # Auto-fetch CRM data from Excel if email entered
    if client_email:
        client_data = get_client_data_from_excel(client_email, "Expanded_CRM_Data.xlsx")
        if client_data:
            st.success("✅ Client found in CRM data!")
            # Auto-fill CRM fields
            customer_name = client_data.get("Customer Name", "")
            product = client_data.get("Product", "")
            needs = client_data.get("Needs", "")
            stage = client_data.get("Stage", "Initial Contact")
            # Show summary
            summary = summarize_client_data(client_data)
            st.markdown("### Client CRM Summary")
            st.write(summary)
        else:
            st.warning("No client record found for this email in the CRM Excel.")
            customer_name, product, needs, stage = "", "", "", "Initial Contact"
    else:
        customer_name, product, needs, stage = "", "", "", "Initial Contact"
    ...
    with st.expander("CRM: AI Strategy Assistant", expanded=False):
        customer_name = st.text_input("Customer/Company Name", value=customer_name)
        product = st.text_input("Product/Service of Interest", value=product)
        needs = st.text_area("Customer Needs / Pain Points", value=needs)
        stage = st.selectbox("Deal Stage", ["Initial Contact", "Discovery", "Demo", "Proposal", "Negotiation", "Closing"], index=["Initial Contact","Discovery","Demo","Proposal","Negotiation","Closing"].index(stage) if stage in ["Initial Contact","Discovery","Demo","Proposal","Negotiation","Closing"] else 0)
